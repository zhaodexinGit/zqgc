...
function confirm_msg(title,str,callback,inst){
    $('<div>'+str+'</div>').dialog({title:title,modal:true, buttons: [
        {
            text: "确定",
...
window.alert = $.alert = function(msg,time){
    ...
    if(time>0){
        $('<div id="temp_alert_box'+randomnum+'">'+msg+'</div>').Tips({title:"操作提示",width:300});
        setTimeout(function(){
        ...
        $('<div id="temp_alert_box">'+msg+'</div>').dialog({title:"操作提示",width:397,modal:true, buttons: [
        {
            text: "确定",
            click: function() { $(this).dialog("close");}
        }]});
