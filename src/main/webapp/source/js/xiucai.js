/**
 * 1、修改9341行 2015-1-13 10:55:08 update "createTime" format to "create_time
 * 2、注释9905行代码
 */
(function(a) {
    var b = function() {
    ...
    function c(r, o) {
        var f = a('<div class="xiucai-qqmsg-mask"></div>');
        var e = a('<div class="xiucai-qqmsg-layer_wrap"/>');
        var l = a(r).attr("id");
        if (!l) {
            l = b()
        }
        f.css(d).appendTo(a(r)).hide().attr("id", l + "_mask");
        var n;
        var m;
        var h = a(r).css("border-top-width");
        var g = a(r).css("border-left-width");
        h = isNaN(parseInt(h)) ? 0 : h;
        g = isNaN(parseInt(g)) ? 0 : g;
        var p = jQuery(r).offset().left + parseInt(g);
        var q = jQuery(r).offset().top + parseInt(h);
        n = parseInt(jQuery(r).width()) + parseInt(jQuery(r).css("padding-right")) + parseInt(jQuery(r).css("padding-left"));
        m = parseInt(jQuery(r).height()) + parseInt(jQuery(r).css("padding-top")) + parseInt(jQuery(r).css("padding-bottom"));
        if (a(r)[0].outerHTML.toLowerCase().indexOf("<body") == -1) {
            f.css("width", n + "px");
            f.css("height", m + "px")
        }
        f.css("left", p + "px");
        f.css("top", q + "px");
        f.css("z-index", o.zIndex);
        var i = "gtl_ico_loading";
        switch (o.type) {
        case "success":
            i = "gtl_ico_succ";
            break;
        ...
        if (o.type && o.type != "loading") {
            e.appendTo(r).attr("id", l + "_loading").empty().append('<span class="xiucai-qqmsg-layer"  id="mode_tips_v2"><span class="' + i + '"></span>' + o.msg + '<span class="gtl_end"></span></span>').css("z-index", o.zIndex + 1)
        } else {
            e.appendTo(r).attr("id", l + "_loading").empty().append('<span class="xiucai-qqmsg-layer"  id="mode_tips_v2"><span class="gtl_ico_clear"></span><span class="' + i + '"></span>' + o.msg + '<span class="gtl_end"></span></span>').css("z-index", o.zIndex + 1)
        }
        var k = q;
        if (o.marginTop) {
            k += parseInt(o.marginTop)
        }
        var j = p;
        e.css("left", (j + ((f.width() - parseInt(e.width())) / 2)).toString() + "px");
        e.css("top", (k + ((f.height() - parseInt(e.height())) / 2)).toString() + "px");
        setTimeout(function() {
            r.hLoading.hide(r, o.onAfterHide)
        },
        o.timeout);
        return e
    }
    a.fn.hLoading = function(g) {
        g = a.extend({},
        a.fn.hLoading.defaults, g || {});
        var e = c(this, g);
        if (typeof(g.onBeforeShow) == "function") {
            g.onBeforeShow()
        }
        e.show();
        var f = e.attr("id").split("_").splice(0, 1);
        a("#" + f.join("_") + "_mask").show();
        if (typeof(g.onAfterShow) == "function") {
            g.onAfterShow()
        }
        return this
    };
    a.fn.hLoading.hide = function(f, e) {
        return f.each(function() {
            var g = a(this);
            a("div.xiucai-qqmsg-mask", g).fadeOut(function() {
                a(this).remove()
            });
            a("div.xiucai-qqmsg-layer_wrap", g).fadeOut(function() {
                a(this).remove()
            });
    ...
    a.hLoading = {
        hide: function() {
            var e = a("body");
            a("div.xiucai-qqmsg-mask", e).fadeOut(function() {
                a(this).remove()
            });
            a("div.xiucai-qqmsg-layer_wrap", e).fadeOut(function() {
                a(this).remove()
            })
        },
        show: function(e) {
...
if(opts.sortName){
//$.extend(_6ae,{sort:opts.sortName,order:opts.sortOrder}); //old
$.extend(_6ae,{sort:replaceStr(opts.sortName),order:opts.sortOrder});//new
}
...
if ($.fn.datebox) {
    ...
    $.fn.datebox.defaults.formatter = function(b) {
        ...
        var c = b.getMonth() + 1;
        var a = b.getDate();
        return e + "-" + (c < 10 ? ("0" + c) : c) + "-" + (a < 10 ? ("0" + a) : a)
    };
    $.fn.datebox.defaults.parser = function(c) {
        ...
        var b = parseInt(e[1], 10);
        var a = parseInt(e[2], 10);
        if (!isNaN(f) && !isNaN(b) && !isNaN(a)) {
            return new Date(f, b - 1, a)
        } else {
            return new Date()
        }
    }
...
$.extend($.fn.validatebox.defaults.rules, {
    ...
    ZIP: {
        validator: function(b, a) {
            return /^[1-9]\d{5}$/.test(b)
        },
        message: "邮政编码不存在"
    },
    QQ: {
        validator: function(b, a) {
            return /^[1-9]\d{4,10}$/.test(b)
        },
        message: "QQ号码不正确"
    },
    mobile: {
        validator: function(b, a) {
            return /^((\(\d{2,3}\))|(\d{3}\-))?13\d{9}$/.test(b)
        },
        message: "手机号码不正确"
    },
    loginName: {
...
var safePassword = function(a) {
    return ! (/^(([A-Z]*|[a-z]*|\d*|[-_\~!@#\$%\^&\*\.\(\)\[\]\{\}<>\?\\\/\'\"]*)|.{0,5})$|\s/.test(a))
};
var idCard = function(k) {
    if (k.length == 18 && 18 != k.length) {
        return false
    }
    var f = k.toLowerCase();
    var c, h = 0,
    ...
    b = "11,12,13,14,15,21,22,23,31,32,33,34,35,36,37,41,42,43,44,45,46,50,51,52,53,54,61,62,63,64,65,71,81,82,91";
    var g = f.match(/^(\d{2})\d{4}(((\d{2})(\d{2})(\d{2})(\d{3}))|((\d{4})(\d{2})(\d{2})(\d{3}[x\d])))$/);
    if (g == null || b.indexOf(g[1]) < 0) {
        return false
    }
    if (g[2].length == 9) {
        f = f.substr(0, 6) + "19" + f.substr(6);
        c = ["19" + g[4], g[5], g[6]].join("-")
    } else {
        c = [g[9], g[10], g[11]].join("-")
    }
    if (!isDateTime.call(c, "yyyy-MM-dd")) {
        return false
    }
    for (var e = 0; e < 17; e++) {
        h += f.charAt(e) * l[e]
    }
    return (g[2].length == 9 || f.charAt(17) == j.charAt(h % 11))
};
var isDateTime = function(g, n) {
    g = g || "yyyy-MM-dd";
    var j = this,
    m = {},
    a = new Date();
    var b = g.split(/[^a-z]+/gi),
    c = j.split(/\D+/g),
    e = g.split(/[a-z]+/gi),
    f = j.split(/\d+/g);
    var k = b.length,
    l = e.length;
    if (k != c.length || l != f.length) {
        return false
    }
    for (var h = 0; h < l; h++) {
        if (e[h] != f[h]) {
    ...
    m.yyyy = q(m.yyyy, m.yy, a.getFullYear(), 9999, 4);
    m.MM = q(m.MM, m.M, a.getMonth() + 1, 12);
    m.dd = q(m.dd, m.d, a.getDate(), 31);
    m.hh = q(m.hh, m.h, a.getHours(), 24);
    m.mm = q(m.mm, m.m, a.getMinutes());
    m.ss = q(m.ss, m.s, a.getSeconds());
    m.ms = q(m.ms, m.ms, a.getMilliseconds(), 999, 3);
    if (m.yyyy + m.MM + m.dd + m.hh + m.mm + m.ss + m.ms < 0) {
        return false
    }
    if (m.yyyy < 100) {
        m.yyyy += (m.yyyy > 30 ? 1900 : 2000)
    }
    a = new Date(m.yyyy, m.MM - 1, m.dd, m.hh, m.mm, m.ss, m.ms);
    var p = a.getFullYear() == m.yyyy && a.getMonth() + 1 == m.MM && a.getDate() == m.dd && a.getHours() == m.hh && a.getMinutes() == m.mm && a.getSeconds() == m.ss && a.getMilliseconds() == m.ms;
    return p && n ? a: p;
    function q(i, o, r, s, t) {
        s = s || 60,
        t = t || 2;
        var d = r;
        if (i != undefined && i != "" || !isNaN(i)) {
            d = i * 1
        }
        if (o != undefined && o != "" && !isNaN(o)) {
            d = o * 1
        }
        return (d == i && i.length != t || d > s) ? -10000 : d
    }
}; (function($) {
...
function _StringFormatStatic() {
    for (var b = 1; b < arguments.length; b++) {
        ...
        arguments[0] = arguments[0].replace(a, arguments[b])
    }
    return arguments[0]
}
...
function convertArray(b) {
    var c = {};
    for (var a in b) {
        if (b[a].name != "__VIEWSTATE") {
            if (typeof(c[b[a].name]) == "undefined") {
...
function MessageOrRedirect(a) {
    if (a) {
        if (a.Data == "-99") {
            ...
            top.$.messager.alert("系统提示", a.Message, "warning")
        }
    }
};
//update str's "createTime" format to "create_time
function replaceStr(str){
	...
	for(var i=0;i<=length;i++){
		...
		if((ch.charCodeAt(0)>=65)&&(ch.charCodeAt(0)<=90)){
			...
			var end=str.substring(i+1,length);
			str=start+"_"+ch.toLowerCase()+end;
			length++;
		}
	}
	return str;
...
function dicStore(path,code){
	var map = new Map();
	jQuery.ajax({ 'url': path+"/base/getDicByTypeCode.htm?code="+code, 'async': false, 'dataType': 'json',
	    'success': function(data){
