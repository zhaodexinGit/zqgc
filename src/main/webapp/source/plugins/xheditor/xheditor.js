    ...
    Xheditor = function(textarea, options) {
        ...
        this.processHTML = function(e, t) {
            function i(e, t, i, s, a, n) {
                ...
                "" !== c ? (s && (c += ' style="' + s + '"'), "<font" + (i ? i: "") + c + (a ? a: "") + ">" + n + "</font>") : e
            }
            function s(e, t, i, s, a) {
                for (var o, l, r = (i ? i: "") + (s ? s: ""), h = [], c = [], d = 0; n.length > d; d++) o = n[d].r,
                l = n[d].t,
                r = r.replace(o,
                function() {
                    return h.push("<" + l + ">"),
            ...
            e = e.replace(/<(td|th)(\s+[^>]*?)?>(\s|&nbsp;)*<\/\1>/gi, "<$1$2>" + (isIE ? "": "<br />") + "</$1>");
                ...
                },
                {
                    ...
                    t: "strike"
                }], o = 0; 2 > o; o++) e = e.replace(/<(span)(\s+[^>]*?)?\s+class\s*=\s*"Apple-style-span"(\s+[^>]*?)?>(((?!<\1(\s+[^>]*?)?>)[\s\S]|<\1(\s+[^>]*?)?>((?!<\1(\s+[^>]*?)?>)[\s\S]|<\1(\s+[^>]*?)?>((?!<\1(\s+[^>]*?)?>)[\s\S])*?<\/\1>)*?<\/\1>)*?)<\/\1>/gi, s),
                ...
                function(e, t, i, s, a) {
                    ...
                    n && urlType && (n = getLocalUrl(n, urlType, urlBase)),
        ...
        this.formatXHTML = function(e, t) {
            ...
            return e.length > j && o(e.substring(j, e.length)),
            n(),
            e = w.join(""),
            w = null,
            e = e.replace(/<(font)(\s+[^>]*?)?>(((?!<\1(\s+[^>]*?)?>)[\s\S]|<\1(\s+[^>]*?)?>((?!<\1(\s+[^>]*?)?>)[\s\S]|<\1(\s+[^>]*?)?>((?!<\1(\s+[^>]*?)?>)[\s\S])*?<\/\1>)*?<\/\1>)*?)<\/\1>/gi, d),
        ...
        this.showEmbed = function(e, t, i, s, a, n, o) {
            var l = $(getLang(t)),
            r = _this.getParent('embed[classid="' + s + '"]'),
            h = $("#xhe" + e + "Url", l),
            c = $("#xhe" + e + "Width", l),
            d = $("#xhe" + e + "Height", l),
            u = $("#xheSave", l);
        ...
        this.showEmbedOrPlayer=function(sType,sHtml,sMime,sClsID,sBaseAttrs,sUploadUrl,sUploadExt)
		{
			var jEmbed=$(getLang(sHtml)),jParent=_this.getParent('embed[classid="'+sClsID+'"]'),jUrl=$('#xhe'+sType+'Url',jEmbed),jWidth=$('#xhe'+sType+'Width',jEmbed),jHeight=$('#xhe'+sType+'Height',jEmbed),jSave=$('#xheSave',jEmbed);
			if(sUploadUrl)_this.uploadInit(jUrl,sUploadUrl,sUploadExt);
			if(jParent.length===1)
			{
				var url=jParent.attr('flashvars');
				jUrl.val(url.substring(url.lastIndexOf("f=")+2,url.length));
			...
			jSave.click(function(){
				...
				if(url!==''&&url!=='http://')
				{
					...
					var sBaseCode='<embed type="' + sMime + '" classid="' + sClsID + '"  src="/html5Cms/ckplayer/ckplayer.swf"  flashvars="xhe_tmpurl"'+sBaseAttrs;
					var aUrl=url.split(' ');
					if(aUrl.length>1)
					{//批量插入
						...
						for(var i in aUrl)
						{
							url=aUrl[i].split('||');
							sEmbed=sTemplate;
							sEmbed=sEmbed.replace('xhe_tmpurl','p=0&f='+url[0])
					...
					{//单URL模式
						...
						if(jParent.length===0)
						{
							...
							jParent=$('embed[flashvars$="#xhe_tmpurl"]',_doc);
						}
						xheAttr(jParent,'flashvars','p=0&f='+url[0]);
        ...
        this.exec = function(e) {
            ...
            case "flash":
                 ...
                 break;
            case "media":
                _this.showEmbedOrPlayer("Media", htmlMedia, "application/x-shockwave-flash", "clsid:6bf52a52-394a-11d3-b153-00c04f79faa6", ' align="middle" wmode="opaque" quality="high" menu="false" play="true" loop="true" allowfullscreen="true"', settings.upMediaUrl, settings.upMediaExt);
                break;
            case "hr":
                _this.pasteHTML("<hr />");
                break;
