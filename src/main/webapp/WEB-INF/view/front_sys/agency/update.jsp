...
<body style="background: none repeat scroll 0 0 #EEEEEE;">
	<form id="formId" action="${ctx}/front_sys/agency/update.htm" method="post">
...
<!-- 简介图片上传 -->
    function dynamicUpload() {
       var url = '${ctx}/front_sys/agency/upload.htm?id='+id
       $.ajax({
          url: url,
		  type :"POST",
	...
	function update() {
		$('#formId').ajaxSubmit(function(data) {
			if (data == "success") {
				alert("保存成功");
				window.location.href = '${ctx}/front_sys/agency/list.htm';
			} else {
				alert("保存失败!");
			}
		});
