...
<form id="mainForm" action="${ctx}/front_sys/agency/list.htm" method="post" style="margin:0;padding:0">
      ...
      <div class="widget-box" style="margin-top:0px">
		<div class="widget-title">
			...
			<span style="margin-top:3px; float:right">
		         <button type="button" onClick="toUrl('${ctx}/front_sys/agency/toAdd.htm')" class="btn btn-info">
...
function toUpdate(id){
	toUrl('${ctx}/front_sys/agency/toUpdate.htm?id='+id);
}

//跳到详情界面
function toDetail(id){
	toUrl('${ctx}/front_sys/agency/toDetail.htm?id='+id);
}

//跳到新增界面
function toAddNlbmm(id){
	toUrl('${ctx}/front_sys/agency/toAdd.htm');
}

//删除记录
function deleteById(id){
	if(confirm("此操作不可撤销，确认删除此记录？")){
		var url='${ctx}/front_sys/agency/delete.htm?id='+id;
		$.ajax({
			url: url,
			async : true,
			type :"GET",
			...
			success:function (data){
				if(data=='success'){
					alert("删除成功");
					window.location.href="${ctx}/front_sys/agency/list.htm?title=${param['title']}&introduction=${param['introduction']}";
				}
			},
			error: function (data){
				alert("删除失败");
